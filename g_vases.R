...
rs<-coo_sample(r,82)
r_angle<-coo_angle_edges(rs)
r_angle<-r_angle %>% as.data.frame()
r_angle <- r_angle %>% as.matrix()
r_n<-scale(r_angle, scale = FALSE)
re<-eigen(crossprod(r_n))
PC<-r_n %*% re$vectors
re$values / sum(re$values)
plot(PC)
escore<-r_n %*% re$vectors
plot(x= PC[1,],y=PC[2,])
##################################
dist_e<-dist(t(escore))
erc<-as.phylo(hclust(dist_e))
plot(x=escore[1,],y=escore[2,])

ti<-tibble(label = c(1:322) %>% as.character(),group = r$group %>% as.character(), type = r$type %>% as.character())
ti_erc<-as_tibble(erc)
...
erc2 <- full_join(ti_erc, ti, by = 'label')
erc2<-full_join(erc2,group_1, by = "label")

for (i in 1:322) {
  drc2$label[i] <- drc2$group[i]
  
}
erc3<-erc2 %>% as.phylo()
erc4<-erc2 %>% as.treedata()
...
cols <- c("red","violet","#6A1B9A","orange","#C5CAE9","#4DD0E1","#00ACC1","#006064","#18FFFF","#B2DFDB","green","#004D40","#A5D6A7","#4CAF50","pink","#C5E1A5","#7CB342","#B2FF59","#76FF03","FFF59D","#F57F17","#BCAAA4","#6D4C41")
...
#pies<-nodepie()
ti<-tibble(label = c(1:322) %>% as.character(),group = r$group %>% as.character(), type = r$type %>% as.character())
ti_drc<-as_tibble(drc)
shp<-c(16,8,25,15,1,17,18)
group_1<-tibble(label = c(1:322) %>% as.character, shpp = c(rep(1,10),2,2,2,rep(1,10),rep(2,10),rep(3,10),rep(4,10),rep(5,10),rep(6,10),rep(7,6),rep(1,10),rep(1,10),rep(2,10),rep(3,4),rep(4,18),rep(5,10),rep(6,10),rep(7,10),rep(1,10),rep(2,8),rep(1,3),rep(1,10),rep(1,10),rep(1,9),rep(1,9),rep(2,8),rep(3,10),rep(4,10),rep(1,8),rep(1,10),rep(1,10),rep(1,8),rep(1,9),rep(2,10),rep(1,9),rep(1,10)) %>% as.character())

drc2 <- full_join(ti_drc, ti, by = 'label')
drc2<-full_join(drc2,group_1, by = "label")
#as.treedata(drc2)
for (i in 1:322) {
  drc2$label[i] <- drc2$group[i]
...
drc3<-drc2 %>% as.phylo()
drc4<-drc2 %>% as.treedata()

#p + geom_inset(bars, width = .08, height = .05, x = "branch") 


cols1<-c("red","firebrick","orange","orange4","gold","gold4","green","green4","springgreen4","cyan","cyan3","blue","blue4","purple","violet","pink","hotpink","brown")
ggtree(drc4,layout = "fan") + 
  scale_shape_manual(values = shp) +
  scale_color_manual(values = cols1)+
  geom_tippoint(aes(color = type, shape = shpp),size=1)
